...
static char		*build_line(t_gnl *env)
{
	...
	//printf("a\n");
	printf("i: %d, env: %d, sub: %s\n", i, env->end, ft_strsub_start(env->datas, i));
	i = i;
...
t_gnl			*init_env(t_gnl **env_param, char shouldDelete)
{
	t_gnl	*env;
...
int				get_next_line(const int fd, char **line)
{
	...
	if (fd < 0 || fd > 255 || !line || (!env && (!(env = init_env(NULL, 0)))))
		return (-1);
	if (!init_env(&env, 1))
		return (0);
	++env->line;
